...
(function () {
  ...
  function cargarAnaliticaProyecto_(cont) {
    if (!datosDetalleActual_) return;
    if (datosDetalleActual_.analitica !== undefined) { cambiarPestana_('cronograma'); return; }
    cambiarPestana_('cronograma'); // repinta ya -- pintarAnaliticaProyecto_ muestra "Cargando..." mientras tanto
    var idAlPedir = proyectoActivoId_;
  ...
  function pintarAnaliticaProyecto_(analitica, tareas) {
    ...
    var conDato = (analitica.por_tarea || []).filter(function (t) {
  ...
  function wireAccionesPestana_(cont, detalle, tareas, puedeGestionar) {
    ...
    if (analiticaCsvBtn) {
      analiticaCsvBtn.addEventListener('click', function () {
        ...
        if (!analitica) return;
        var filas = [['Tarea', 'Lead time (días)', 'Cycle time (días)', 'Tiempo en bloqueo (días)', 'Tiempo en revisión (días)']];
        analitica.por_tarea.forEach(function (t) {
