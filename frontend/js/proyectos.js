...
(function () {
  ...
  function pintarCronogramaPlan_(detalle, tareas, ctxEdicion, rendimiento) {
    ...
    var controles = '<div class="sigso-py-ded-controles">' + zoomBtns +
      ...
      rutaCriticaBtn + dependenciasBtn + agruparBtn + filtroHitoSelect + expandirTodoBtn +
  ...
  function wireAccionesPestana_(cont, detalle, tareas, puedeGestionar) {
    ...
    var descargarXlsx = cont.querySelector('.js-py-descargar-xlsx');
    if (descargarXlsx) {
      descargarXlsx.addEventListener('click', function () {
        var txt = descargarXlsx.textContent;
        descargarXlsx.disabled = true;
        descargarXlsx.innerHTML = '<span class="sigso-spinner"></span>Generando…';
        api_('descargarLibroProyecto', { proyecto_id: detalle.proyecto.proyecto_id }).then(function (r) {
          if (!r || !r.ok) {
          ...
          descargarXlsx.disabled = false;
          descargarXlsx.textContent = txt;
        });
      });
    }
