...
(function () {
  ...
  function pintarMiDedicacion_(cont, toggle, tareas) {
    ...
    wireMiTrabajoToggle_(cont, { tareas: tareas });
    wireDedicacionNav_(cont, function () { pintarMiDedicacion_(cont, toggle, tareas); });
  }
  ...
  function pintarCronogramaDedicacion_(detalle, tareas, bitacora, rendimiento) {
    ...
    var filasHtml = filas.map(function (a) {
      ...
      // v10 (Fase G3): meta + ritmo, como segunda línea de la etiqueta --
      // "16 imágenes · 16/día" una vez terminada; solo "16 imágenes" antes
      // (dividir por el avance parcial seria inventar un ritmo). El ritmo
      // sale de `rendimiento` (por proyecto); la meta en si misma ya viene
      // en la propia tarea (listarTareas/listarMisTareas), asi que en "Mi
      // dedicación" (varios proyectos, sin rendimiento) igual se ve la
      // meta, solo que sin ritmo -- mezclar imágenes con piezas en un solo
      // promedio no tendria sentido.
      ...
      var metaTxt = a.meta_cantidad
        ...
        : '';
      // v10 (Fase G3, "vista transversal por recurso"): si la fila viene de
      // "Mi dedicación" (varios proyectos a la vez), a.proyecto_nombre
      // existe y reemplaza al responsable (que en esa vista siempre soy yo).
  ...
  function wireAccionesPestana_(cont, detalle, tareas, puedeGestionar) {
    ...
    wireDedicacionNav_(cont, function () { cambiarPestana_('cronograma'); });
