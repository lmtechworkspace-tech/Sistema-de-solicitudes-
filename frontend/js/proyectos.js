...
(function () {
  ...
  // v16.9: mismo botón "Descargar Excel" en dos lugares (Resumen y, ahora,
  // el propio Cronograma > Plan -- así lo pedía el mockup original: el
  // "[↓ Descargar]" vive junto a Expandir/Contraer todo, no solo en Resumen).
  // Se extrae el click UNA vez para no duplicar el manejo de spinner/error.
  function wireDescargarXlsxBtn_(btn, proyectoId) {
    if (!btn) return;
    btn.addEventListener('click', function () {
      var txt = btn.textContent;
      btn.disabled = true;
      btn.innerHTML = '<span class="sigso-spinner"></span>Generando…';
      api_('descargarLibroProyecto', { proyecto_id: proyectoId }).then(function (r) {
        if (!r || !r.ok) {
          Componentes.aviso({ texto: (r && r.message) || 'No se pudo generar el archivo.', tipo: 'error' });
          return;
        }
        descargarBase64Proyecto_(r.data.xlsx_base64, r.data.filename || 'Proyecto.xlsx',
  ...
  function pintarResumen_(detalle, tareas, puedeGestionar) {
    ...
    // R-01: el PDF es para leer; el Excel es para seguir trabajando encima.
    var descargarXlsx = Componentes.boton({ texto: 'Descargar Excel', variante: 'secundario', clase: 'js-py-descargar-xlsx' });
    ...
    var acciones = puedeGestionar
      ? '<div class="sigso-py-acciones">' +
          ...
          descargarPdf + descargarXlsx + configurarInforme +
  ...
  var SECCIONES_INFORME_ = [
    ...
    { valor: 'gantt', texto: 'Carta Gantt (barras por semana + ejecución día a día)' },
  ...
  // set de secciones profesional por defecto (portada, resumen ejecutivo,
  // KPIs, salud, desviaciones, Carta Gantt de barras, workload, hitos, riesgos,
  ...
  // v15.8 (A1): 'narrativa' (Resumen ejecutivo) entra al set por defecto,
  // primero -- responde "cómo va, qué está mal, qué hacer" en un párrafo,
  // antes de cualquier tabla. Antes solo aparecía si el usuario entraba a
  // "Configurar informe", así que el botón que casi todos aprietan caía
  // directo a los números. El orden final lo fija REPORTE_SECCIONES_
  // DISPONIBLES_ en el backend (narrativa va tras la portada igual).
  var CRONOGRAMA_REPORTE_SECCIONES_ = [
    'portada', 'narrativa', 'ficha', 'kpis', 'salud', 'desviaciones', 'gantt',
  ...
  // Agrupar el Gantt por hito. APAGADO por defecto: la vista cronologica es
  // la que la gente ya conoce y responde "que se vence esta semana".
  // Agrupada responde otra cosa -- "de que se compone el proyecto" -- y esa
  // se elige, no se impone.
  var agruparPorHito_ = false;
  // v16.8: filtro por hito en la vista Plan. '' = todos; un hito_id = solo ese
  // hito; '__sinhito__' = solo las tareas sin hito. Sobrevive al repintado
  // como el resto del estado del Plan.
  var filtroHitoPlan_ = '';
  // Que grupos estan plegados, por hito_id. Sobrevive al repintado (zoom,
  // cambio de sub-vista) porque plegar y que se despliegue solo al mover el
  // zoom seria pelearse con la herramienta.
  var gruposPlegados_ = {};
  ...
  function dibujarConectoresGantt_(cont) {
    ...
    Object.keys(barras).forEach(function (id) {
      ...
      // Sin barra de origen no hay conector. Pasa en dos casos, y en los dos
      // omitirlo es lo correcto: la tarea de la que se depende no tiene fecha
      // comprometida (no esta en el Gantt), o su grupo esta plegado. Dibujar
      // una linea que sale de la nada seria peor que no dibujarla.
      if (!origen) return;
  ...
  function pintarCronogramaPlan_(detalle, tareas, ctxEdicion, rendimiento) {
    ...
    // v16.8: filtro por hito -- acota tareas E hitos a la selección. Si el hito
    // elegido ya no existe (se borró), se ignora el filtro en vez de mostrar
    // una pantalla vacía sin explicación.
    var idsHitoExisten_ = {};
    hitosTodos.forEach(function (h) { idsHitoExisten_[h.hito_id] = true; });
    if (filtroHitoPlan_ && filtroHitoPlan_ !== '__sinhito__' && !idsHitoExisten_[filtroHitoPlan_]) filtroHitoPlan_ = '';
    if (filtroHitoPlan_ === '__sinhito__') {
      hitos = [];
      tareasConFecha = tareasConFecha.filter(function (a) { return !a.hito_id || !idsHitoExisten_[a.hito_id]; });
    } else if (filtroHitoPlan_) {
      hitos = hitosTodos.filter(function (h) { return h.hito_id === filtroHitoPlan_; });
      tareasConFecha = tareasConFecha.filter(function (a) { return a.hito_id === filtroHitoPlan_; });
    }
    ...
    var hitosPorId_ = {};
    hitos.forEach(function (h) { hitosPorId_[h.hito_id] = h; });
    var basePorTarea = {};
    ...
    var porCompromiso_ = function (a, b) {
    ...
    var filasTareas;
    if (!agruparPorHito_) {
      filasTareas = tareasConFecha.slice().sort(porCompromiso_).map(filaTareaGantt_).join('');
    } else {
      // Agrupado: cada hito con SUS tareas debajo, y al final las que no
      // cuelgan de ninguno. El orden de los hitos es por fecha objetivo, que es
      // el orden en que el proyecto los va a cruzar.
      var porHito_ = {};
      var sinHito_ = [];
      tareasConFecha.forEach(function (a) {
        if (a.hito_id && hitosPorId_[a.hito_id]) (porHito_[a.hito_id] = porHito_[a.hito_id] || []).push(a);
        else sinHito_.push(a);
      });

      var trozos = [];
      hitos.slice().sort(function (x, y) {
    ...
    function filaGrupoGantt_(hito, suyas) {
      ...
      var chevron = hito
        ? '<button type="button" class="sigso-py-gantt-grupo__toggle js-py-gantt-grupo" data-hito="' +
            ...
            'title="' + (plegado ? 'Mostrar' : 'Ocultar') + ' las tareas de ' + Componentes.escaparHtml(nombre) + '">' +
            (plegado ? '▸' : '▾') + '</button>'
    ...
    var dependenciasBtn = hayRutaCritica
      ...
      : '';
    // Solo si hay hitos: agrupar por algo que no existe no agrupa nada.
    ...
    var expandirTodoBtn = (hitos.length && agruparPorHito_)
      ...
      : '';
    // v16.8: filtro por hito -- solo si hay al menos un hito. Muestra "Sin hito"
    // como opción únicamente cuando existen tareas sin hito, para no ofrecer un
    // filtro que dejaría la pantalla vacía.
    var haySinHito = tareas.some(function (a) { return a.fecha_compromiso && (!a.hito_id || !idsHitoExisten_[a.hito_id]); });
    var filtroHitoSelect = '';
    if (hitosTodos.length) {
      var opsHito = hitosTodos.map(function (h) { return { valor: h.hito_id, texto: h.nombre }; });
      if (haySinHito) opsHito.push({ valor: '__sinhito__', texto: 'Sin hito' });
      filtroHitoSelect = Componentes.campoSelect({
        id: 'py-gantt-filtro-hito', valor: filtroHitoPlan_, placeholder: 'Todos los hitos',
        claseCampo: 'sigso-py-ded-ctrl', claseInput: 'js-py-gantt-filtro-hito', opciones: opsHito
      });
    }
    // v16.9: "[↓ Descargar]" en el propio Cronograma -- el mockup original
    // lo pedía acá, junto a Expandir/Contraer todo, no solo en la pestaña
    // Resumen. Mismo endpoint (descargarLibroProyecto), mismo botón
    // reutilizado (wireDescargarXlsxBtn_).
    var descargarXlsxBtn = Componentes.boton({ texto: 'Descargar Excel', variante: 'secundario', clase: 'js-py-gantt-descargar-xlsx', tipo: 'button' });
    var controles = '<div class="sigso-py-ded-controles">' + zoomBtns +
      ...
      rutaCriticaBtn + dependenciasBtn + agruparBtn + filtroHitoSelect + expandirTodoBtn + descargarXlsxBtn +
    ...
    return controles +
      '<div class="sigso-py-gantt-scroll">' +
        '<div class="sigso-py-gantt-body">' +
          ...
          '</div>' +
          // Agrupado, los hitos YA son las cabeceras de cada grupo: repetirlos
          // arriba los mostraria dos veces.
          (agruparPorHito_ ? '' : filasHitos) + filasTareas +
  ...
  function wireAccionesPestana_(cont, detalle, tareas, puedeGestionar) {
    ...
    wireDescargarXlsxBtn_(cont.querySelector('.js-py-descargar-xlsx'), detalle.proyecto.proyecto_id);
    // v16.9: el mismo botón, ahora también en Cronograma > Plan (el mockup
    // original lo ponía junto a Expandir/Contraer todo, no solo en Resumen).
    wireDescargarXlsxBtn_(cont.querySelector('.js-py-gantt-descargar-xlsx'), detalle.proyecto.proyecto_id);
    ...
    // v16.8: filtro por hito -- repinta desde caché (sin red), como el resto.
    var filtroHitoSel = cont.querySelector(".js-py-gantt-filtro-hito");
    if (filtroHitoSel) {
      filtroHitoSel.addEventListener("change", function () {
        filtroHitoPlan_ = this.value;
        cambiarPestana_('cronograma');
      });
    }

    // Plegar/desplegar un grupo. Delegacion sobre el contenedor: las
    // cabeceras se re-crean en cada repintado y un solo listener las cubre.
    var cajaGantt = cont.querySelector(".sigso-py-gantt-scroll");
    if (cajaGantt) {
      cajaGantt.addEventListener("click", function (ev) {
        var b = ev.target.closest(".js-py-gantt-grupo");
        if (!b) return;
        var id = b.getAttribute("data-hito");
        if (gruposPlegados_[id]) delete gruposPlegados_[id]; else gruposPlegados_[id] = true;
        cambiarPestana_('cronograma');
      });
    }
