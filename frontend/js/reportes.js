...
(function () {
  ...
  function filtrosParaCabecera(reporte, opciones, valores) {
    ...
    return lista.map(function (nombre) {
      ...
      // 'todo' no es un corte, es la ausencia de uno: anunciarlo en la
      // cabecera del documento seria ensuciarla con algo que no dejo nada fuera.
      ...
      return { etiqueta: etiquetaFiltro_(reporte, nombre), valor: texto };
    }).filter(Boolean);
  }
  /**
   * El nombre visible de un filtro, con la afinacion que el reporte pida.
   *
   * Existe por "Período": segun el reporte, el mismo select corta por una
   * fecha distinta (cuando ENTRO el item, cuando se ENTREGO). Un select que
   * dice solo "Período" deja a quien lo usa adivinando cual de las dos, y
   * dos personas leyendo el mismo numero pueden entender cosas distintas.
   * Asi que el reporte que corta por algo concreto lo escribe:
   * `etiquetaPeriodo: "Ítems creados en"`.
   */
  ...
  function pintarFiltros(reporte, opciones, valores) {
    ...
    var campos = lista.map(function (f) {
      // Sin valor elegido, se marca 'todo' y NO la primera opcion. Un <select>
      // siempre tiene algo seleccionado: si se dejara al navegador elegir, el
      // desplegable diria "Este mes" mientras el reporte muestra todo, y quien
      // lo lea creera que esta viendo un corte que no se aplico.
  ...
  // APLICAR LOS FILTROS
  // ==========================================================================
  // Esto vive AQUI y no en cada módulo por el mismo motivo que la agregación
  // de cumplimiento: si Gerencia resolviera "este trimestre" de una forma y
  // Jefatura de otra, un jefe y Gerencia verían números distintos del mismo
  // equipo y nadie sabría cuál creer.

  /**
   * Traduce un período a un rango de fechas `YYYY-MM-DD`.
   *
   * Se trabaja con el prefijo de 10 caracteres y NO con objetos Date: los
   * ítems traen la fecha como texto ISO, y compararlos como texto evita de
   * raíz que un ítem del día 1 caiga en el mes anterior porque el navegador
   * está en otro huso. Es el mismo criterio que usa el backend.
   */
  function rangoDePeriodo(periodo, hoyISO) {
    var hoy = String(hoyISO || new Date().toISOString()).slice(0, 10);
    var anio = hoy.slice(0, 4);
    ...
    if (periodo === 'trimestre') {
      ...
      return {
        desde: anio + '-' + dos_(primerMes) + '-01',
        hasta: finDeMes_(anio, primerMes + 2)
      };
    }
    if (periodo === 'anio') return { desde: anio + '-01-01', hasta: anio + '-12-31' };
    return { desde: '', hasta: '' };   // 'todo', vacío o desconocido: sin corte
  }

  function dos_(n) { return (n < 10 ? '0' : '') + n; }

  function finDeMes_(anio, mes) {
    // Día 0 del mes siguiente = último del mes pedido, y así no hay que
    // saberse cuáles tienen 31 ni acordarse de los bisiestos.
    var d = new Date(Date.UTC(Number(anio), mes, 0));
    return d.toISOString().slice(0, 10);
  }

  /**
   * Aplica a una lista de ítems los filtros que la persona eligió.
   *
   * `opciones.campoFecha` dice SOBRE QUÉ FECHA corta el período. No hay un
   * valor por defecto razonable para todos: cortar un reporte de
   * cumplimiento por la fecha de entrega dejaría fuera todo lo que sigue
   * abierto, y su columna "Abiertos" quedaría siempre en cero — el reporte
   * seguiría dibujándose, diciendo otra cosa. Por eso cada reporte lo
   * declara y la etiqueta del filtro lo dice en pantalla.
   *
   * Un ítem SIN esa fecha queda fuera cuando hay período elegido: no se
   * puede afirmar que cae dentro de un rango que no tiene.
   */
  function filtrarItems(items, valores, opciones) {
    ...
    // Un desde/hasta explícito manda sobre el período: si la persona
    // escribió fechas, son las suyas.
    ...
    var campoFecha = opciones.campoFecha;
    var hayRango = !!(rango.desde || rango.hasta);
    ...
    return items.filter(function (i) {
      ...
      if (!coincide_(i, campos.area, valores.area)) return false;
      if (!coincide_(i, campos.responsable, valores.responsable)) return false;
      if (!coincide_(i, campos.estado, valores.estado)) return false;
      return true;
    });
  }
  ...
  /**
   * Arma las opciones de los selects a partir de los propios ítems.
   *
   * Se sacan de los datos y no de un catálogo para que no aparezca en el
   * desplegable un área sin nada que mostrar: elegirla daría un reporte
   * vacío que parece un error del sistema.
   */
  ...
  /**
   * Escribe y descarga un CSV a partir de filas (array de arrays).
   *
   * Existe aparte de descargarCsv_ porque no todas las pantallas exportan
   * desde una <table>: la Bandeja, por ejemplo, pinta tarjetas agrupadas y
   * arma sus filas a mano. Lo que NO puede volver a divergir es COMO se
   * escribe el archivo -- el BOM, el fin de linea y el entrecomillado.
   */
  function descargarCsvDeFilas(filas, nombre) {
    var lineas = (filas || []).map(function (fila) {
      return fila.map(function (celda) {
        return '"' + String(celda == null ? '' : celda).replace(/"/g, '""') + '"';
      }).join(',');
    });
  ...
  window.SigsoReportes = {
    ...
    opcionesDeItems: opcionesDeItems,
    kpis: kpis,
    tabla: tabla,
    ranking: ranking,
    tendencia: tendencia,
    ...
    descargarCsvDeFilas: descargarCsvDeFilas
  };
})();
