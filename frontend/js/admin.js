...
(function () {
    ...
    llamarApi(window.SIGSO_CONFIG.BACKOFFICE_URL, 'listarCatalogo', { tipo: tipo }).then(function (respuesta) {
      ...
      document.querySelectorAll('[data-editar]').forEach(function (fila) {
        fila.addEventListener('click', function () {
          var registro = registroDeFila_(fila);
          if (!registro) return;
    ...
    llamarApi(window.SIGSO_CONFIG.BACKOFFICE_URL, 'listarCuentasPortal', {}).then(function (respuesta) {
      ...
      document.querySelectorAll('[data-cuenta]').forEach(function (fila) {
        fila.addEventListener('click', function (e) {
          ...
          var cuenta = filasCuentasPortal_[Number(fila.getAttribute('data-cuenta'))];
          if (!cuenta) return;
  ...
  function renderTablaCuentas_(cuentas) {
    ...
    filasCuentasPortal_ = cuentas;
    var filas = cuentas.map(function (c, indice) {
  ...
  function renderPausasConfig_() {
    llamarApi(window.SIGSO_CONFIG.BACKOFFICE_URL, 'listarPausasConfig', {}).then(function (respuesta) {
      ...
      document.querySelectorAll('[data-editar-config-pausas]').forEach(function (fila) {
        fila.addEventListener('click', function () {
          var cfg = filasPausasConfig_[Number(fila.getAttribute('data-editar-config-pausas'))];
          if (cfg) abrirFormPausasConfig_(cfg);
        });
      });
    }).catch(mostrarErrorAdmin_);
  }

  // Idem para las cuentas del portal: guardan correos y listas de modulos,
  // justo el tipo de dato donde un & mal escapado se nota tarde.
  var filasCuentasPortal_ = [];

  // Mismo motivo que filasTablaAdmin_: el registro no vuelve a pasar por el
  // DOM, asi que no hay escape que se pueda quedar corto.
  var filasPausasConfig_ = [];
  ...
  function tablaPausasConfig_(filas) {
    ...
    filasPausasConfig_ = filas;
    var cuerpo = filas.map(function (c, indice) {
      var activo = c.activo === true || c.activo === 'TRUE';
      return '<tr data-editar-config-pausas="' + indice + '">' +
  ...
  // Las filas de la tabla que se esta mostrando. Solo hay UNA a la vez: los
  // tres renderizadores que usan renderTabla_ (catalogos, usuarios, logs)
  // reemplazan el mismo #admin-contenido.
  //
  // Antes el registro completo viajaba SERIALIZADO dentro del atributo
  // data-editar, y de ahi salia un defecto silencioso: el atributo escapaba
  // las comillas simples pero no el `&`, asi que el navegador decodificaba de
  // vuelta cualquier cosa con forma de entidad. Comprobado en el navegador:
  //
  //   guardado    "Bonos &amp; Comisiones"
  //   recuperado  "Bonos & Comisiones"
  //
  // Y eso no se quedaba en pantalla: al pinchar la fila el formulario se
  // precargaba con el valor corrompido, y al guardar se reescribia el
  // catalogo con el texto mal. Nadie se enteraba.
  //
  // Guardar un INDICE en vez del registro no arregla el escape: quita la
  // vuelta por el DOM entera, que es donde estaba el problema. (No es XSS:
  // las comillas simples SI se escapaban, y se comprobo que un <script> en el
  // dato viaja como texto.)
  var filasTablaAdmin_ = [];

  /** El registro de la fila pinchada, o null si la tabla ya se repinto. */
  function registroDeFila_(tr) {
    return filasTablaAdmin_[Number(tr.getAttribute('data-editar'))] || null;
  }

  function renderTabla_(campos, filas) {
    filasTablaAdmin_ = filas || [];
    ...
    var cuerpo = filas.map(function (fila, indice) {
      var celdas = campos.map(function (c) {
        ...
        // Un campo ausente se pinta VACIO, no con la palabra "undefined":
        // String(undefined) la escribia tal cual en la celda.
        var valor = fila[c.nombre];
        return '<td>' + Componentes.escaparHtml(valor === undefined || valor === null ? '' : String(valor)) + '</td>';
      }).join('');
      return '<tr data-editar="' + indice + '">' + celdas + '</tr>';
    }).join('');
