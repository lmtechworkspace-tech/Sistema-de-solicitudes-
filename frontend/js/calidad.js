...
(function () {
  ...
  function pintarDatosPersona_(data) {
    ...
    var acciones = puedeGestionar_
      ? '<div class="sgc-acciones">' +
        ...
        '</div>'
      : '';
    // v16.4: antes era una sola grilla plana de 8 campos sin ningun orden --
    // "Cargo" (puede traer una clausula entre parentesis, ej. "Prevencionista
    // de Riesgos (Servicios a Clientes segun DOC-05)") caia en una columna de
    // ~11rem y se envolvia en 3 lineas, dejando huecos en las columnas cortas
    // de al lado. Ademas "Cargo" y "Tipo" ya se ven arriba (subtitulo y badge
    // junto al nombre) -- repetirlos aca no agrega nada, solo genera el
    // desborde. Se saca "Cargo" de la grilla y se agrupan los campos que
    // quedan en dos bloques con sentido: lo laboral y el contacto/reporte.
    return '<div class="sgc-datos">' +
      '<div class="sgc-seccion">' +
        '<h3 class="sgc-seccion__titulo">Información laboral</h3>' +
        '<dl class="sgc-ficha">' +
      ...
      '</div>' +
      '<div class="sgc-seccion">' +
        '<h3 class="sgc-seccion__titulo">Contacto y jefatura</h3>' +
        '<dl class="sgc-ficha">' +
  ...
  function cuerpoEvolucionCobertura_(data) {
    ...
    if (!fotos.length) {
      // DOS vacíos distintos, y confundirlos sería caro: si falta la hoja,
      // el pase diario intenta escribirla cada día, falla, y el error se
      // queda en el registro. La pantalla diría "aún no hay fotos" para
      // siempre y nadie se enteraría.
      var faltaLaHoja = (data && data.hoja_lista === false);
      return SigsoReportes.kpis([
        { etiqueta: 'Indicador interno hoy', valor: (actual.pct_listo || 0) + '%' }
      ]) +
      (faltaLaHoja ? Componentes.alerta(
        'Falta un paso de instalación: la hoja SGC_COBERTURA_HISTORICO no existe en la planilla, así que la foto semanal no se puede guardar. Hay que ejecutar el Instalador (proyecto SETUP) una vez. Mientras tanto el indicador de hoy es correcto, pero no se está registrando su historia.', 'error') : '') +
