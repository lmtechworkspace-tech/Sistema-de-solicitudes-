...
var COLOR_MARCA_REPORTES_ = '#6D5DF6';
...
function encabezadoReporteActividades_(titulo) {
  return '<table width="100%" style="border-collapse:collapse;"><tr>' +
    '<td style="background:' + COLOR_MARCA_REPORTES_ + ';padding:14px 22px;">' +
    '<span style="color:#ffffff;font-size:20px;font-weight:bold;letter-spacing:0.3px;">SIGSO</span>' +
    '<span style="color:#E7E4FF;font-size:15px;margin-left:10px;">' + escaparHtml_(titulo) + '</span>' +
    '</td></tr></table>';
}
...
function construirHtmlReporteActividades_(reporte) {
  ...
  var encabezadoTabla = '<tr>' + reporte.columnas.map(function (c) {
    return '<th style="padding:6px 8px;text-align:left;border-bottom:2px solid #0F172A;">' + escaparHtml_(c.etiqueta) + '</th>';
  }).join('') + '</tr>';

  var filasHtml = reporte.filas.map(function (fila) {
    return '<tr>' + reporte.columnas.map(function (c) {
      return '<td style="padding:6px 8px;border-bottom:1px solid #E5E8EF;">' + escaparHtml_(formatearCeldaReporteActividades_(fila[c.campo])) + '</td>';
    }).join('') + '</tr>';
  }).join('');

  return '<!DOCTYPE html><html><head><meta charset="utf-8"></head>' +
    '<body style="margin:0;font-family:Arial,Helvetica,sans-serif;color:#0F172A;font-size:12px;line-height:1.4;">' +
    encabezadoReporteActividades_(titulo) +
    '<div style="padding:20px 22px;">' +
    '<p style="margin:0 0 4px;font-size:11px;color:#8A93A5;">Generado el ' + escaparHtml_(formatearFechaLegible_(new Date())) + '</p>' +
    '<p style="margin:0 0 14px;">' + resumenHtml + '</p>' +
    '<table width="100%" style="border-collapse:collapse;">' + encabezadoTabla + filasHtml + '</table>' +
    (reporte.filas.length === 0
      ? '<p style="color:#8A93A5;margin-top:12px;">Sin datos para los filtros elegidos.</p>'
      : '') +
    '</div></body></html>';
}
...
function seccionActaReunion_(titulo, filas, campoExtra) {
  if (!filas.length) {
    return '<h2 style="font-size:15px;margin:18px 0 6px;">' + escaparHtml_(titulo) + '</h2>' +
      '<p style="color:#8A93A5;margin:0;">Nada que reportar.</p>';
  }
...
function construirHtmlActaReunion_(acta) {
  return '<!DOCTYPE html><html><head><meta charset="utf-8"></head>' +
    '<body style="margin:0;font-family:Arial,Helvetica,sans-serif;color:#0F172A;font-size:13px;line-height:1.45;">' +
    encabezadoReporteActividades_('Acta de reunión de seguimiento') +
    '<div style="padding:20px 22px;">' +
    '<p style="margin:0 0 14px;font-size:11px;color:#8A93A5;">Generada el ' + escaparHtml_(formatearFechaLegible_(new Date(acta.generado_en))) + '</p>' +
