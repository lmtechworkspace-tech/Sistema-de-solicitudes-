...
#
# Dónde queda: /opt/sigso/chrome/chrome-headless-shell (enlace al binario de la
# versión vigente). logica/pdfChromium.js lo busca ahí; SIGSO_CHROME_PATH lo
# puede reemplazar.
set -euo pipefail

APP=/opt/sigso
DESTINO="$APP/chrome"
ENLACE="$DESTINO/chrome-headless-shell"
...
mkdir -p "$DESTINO"
SALIDA=$(cd "$APP" && node node_modules/@puppeteer/browsers/lib/main-cli.js install "chrome-headless-shell@$VERSION" --path "$DESTINO")
